...
Fecha Fecha::operator++(int i){
    Fecha temp = *this; //Cogemos el objeto que da el operador de arriba
    ++(*this); //incrementamos
    return temp; //devolvemos
}

//Sumamos fecha mas entero (f+5)
...
//Version Global sumamos entero + fecha (5+f)
...
ostream &operator<<(ostream &s, const Fecha &f){
    ...
    s << f.dia << " " <<  nombreMes[f.mes] << " " << f.anio;
    return s;
}
